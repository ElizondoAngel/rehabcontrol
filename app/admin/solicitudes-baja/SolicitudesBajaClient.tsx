...
export default function SolicitudesBajaClient({ solicitudesIniciales, adminNombre }: Props) {
    ...
    try {
      ...
      if (res.ok) {
        setSolicitudes(prev =>
          prev.map(s =>
            s.id === id
              ? { ...s, estado, fecha_revision: new Date().toISOString() }
              : s
          )
        )
    ...
    <>
      ...
      <div className="sidebar">
        ...
        <div className="sb-role">
          ...
          <div>
            <div className="sb-role-name">{adminNombre}</div>
            <div className="sb-role-sub">Acceso Total</div>
          </div>
        </div>
        <nav className="sb-nav">
          {[
            { icon: '🏠', label: 'Panel General',      href: '/admin/dashboard' },
            { icon: '👥', label: 'Usuarios y Roles',   href: '/admin/usuarios' },
            { icon: '📋', label: 'Expedientes',         href: '#' },
            { icon: '💳', label: 'Finanzas',            href: '#' },
            { icon: '📊', label: 'Reportes',            href: '#' },
            { icon: '🔍', label: 'Logs de Auditoría',  href: '/admin/logs' },
            { icon: '⚠️', label: 'Solicitudes de Baja', href: '/admin/solicitudes-baja', active: true },
            { icon: '⚙️', label: 'Configuración',       href: '#' },
          ].map(n => (
            <a key={n.label} href={n.href} className={(n as any).active ? 'active' : ''}>
      ...
      <div className="main">
        ...
        <div className="content">
          ...
          {pendientesCount > 0 ? (
            ...
            <div className="alerta-pendientes none">
            ...
            {filtradas.length === 0 ? (
              ...
              filtradas.map(s => (
                <div className="t-row" key={s.id}>
                  ...
                  <div className="t-cell" style={{ fontSize: 12, color: 'var(--muted)' }}>
                    {s.email}
                  </div>
                  <div>
                  ...
                  <div className="acciones">
                    <button className="btn-detalle" onClick={() => setModalDetalle(s)}>
                      Ver
                    </button>
                    {s.estado === 'pendiente' && (
                      <>
        ...
        <div className="modal-overlay" onClick={() => setModalDetalle(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="detail-row">
              ...
              <span className="detail-val">{modalDetalle.nombre}</span>
            </div>
            <div className="detail-row">
              <span className="detail-label">Correo</span>
              <span className="detail-val">{modalDetalle.email}</span>
            </div>
            <div className="detail-row">
              <span className="detail-label">Estado</span>
              <span className="detail-val">
                <span className={`estado-badge estado-${modalDetalle.estado}`}>
                  {modalDetalle.estado}
                </span>
              </span>
            </div>
            <div className="detail-row">
              ...
              <span className="detail-val">
                {formatFecha(modalDetalle.created_at)} {formatHora(modalDetalle.created_at)}
              </span>
            </div>
            {modalDetalle.fecha_revision && (
              <div className="detail-row">
                <span className="detail-label">Fecha revisión</span>
                <span className="detail-val">{formatFecha(modalDetalle.fecha_revision)}</span>
              </div>
            )}

            <div style={{ marginTop: 14, fontSize: 11, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '.07em', fontWeight: 600 }}>
              Motivo
            </div>
            <div className="motivo-box">
              {modalDetalle.motivo ?? 'Sin motivo especificado'}
            </div>

            <div className="modal-actions">
              <button className="btn-cancel-modal" onClick={() => setModalDetalle(null)}>
                Cerrar
              </button>
              {modalDetalle.estado === 'pendiente' && (
                <>
                  <button
                    className="btn-rechazar"
                    onClick={() => resolver(modalDetalle.id, 'rechazada')}
                    disabled={procesando === modalDetalle.id}
                  >
                    {procesando === modalDetalle.id ? 'Procesando...' : '✗ Rechazar'}
                  </button>
                  <button
                    className="btn-aprobar"
                    onClick={() => resolver(modalDetalle.id, 'aprobada')}
                    disabled={procesando === modalDetalle.id}
                  >
